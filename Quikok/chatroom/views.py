...
def chat(request, user_url):
    ...
    if len(user_nick_temp)< 1:
        user_nick_temp = user_profile.objects.filter(username= username)
        if len(user_nick_temp)< 1:
            print("當前使用者:"+ username + "不是老師也不是學生，可能是測試帳號或漏加帳號")
        else:
            user_nick_temp = user_profile.objects.get(username= username) 
            user_is_teacher = "F" # 之後在html 依據user是學生還是老師切換顯示內容用的
    else:
        user_nick_temp = vendor_profile.objects.get(username= username)
        user_is_teacher = "T"
...
# 再用 auth_username 去 account的兩個table找 nickname
    ...
    roomid_and_friend = list(zip(roomid_list, friend_list))
    ...
    return render(request, 'chatroom/room.html', {
        ...
        'roomid_and_friend':roomid_and_friend
    })
